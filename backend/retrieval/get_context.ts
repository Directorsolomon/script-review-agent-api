    ...
    try {
      ...
      const [vectorResults, bm25Results, scriptVectorResults, scriptBM25Results] = await Promise.all([
        ...
        vector.searchDocs({
          ...
          console.warn("Vector search on docs failed:", error);
          return { results: [] };
        }),
        ...
        vector.searchDocsBM25({
          ...
          console.warn("BM25 search on docs failed:", error);
          return { results: [] };
        }),
        ...
        vector.searchScript({
          ...
          console.warn("Vector search on script failed:", error);
          return { results: [] };
        }),
        ...
        vector.searchScriptBM25({
          ...
          console.warn("BM25 search on script failed:", error);
          return { results: [] };
        }),
      ]);
      ...
      const docs = docIds.length > 0 ? await db.queryAll`
